...
import { assignTenantToSpace, loadData } from '@/lib/store';
...
export default function SelectTenantScreen() {
  ...
  const handleSelectTenant = async (tenant: Tenant) => {
    try {
      const targetRoom = address?.rooms.find((r) => r.id === roomId);
      const availableSpace = targetRoom?.spaces.find((s) => !s.tenant && s.status !== 'inactive');
      if (!availableSpace) return;

      await assignTenantToSpace(
        projectId as string,
        addressId as string,
        roomId as string,
        availableSpace.id,
